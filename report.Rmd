...
Because the CV estimate is dependent on the splitting of the $k$-folds, we would like to visualize the variance of our CV estimate using different random splitting of the data. Therefore, we perform the following operations for 10 iterations:
For each of the 7 models we get from the previous Best Subset Selection, compute the CV estimate 10 times using different random splitting

Plot the CV estimates for the 7 models
...
```{r compare_tab, echo=FALSE, out.width='90%', fig.align="center"}
knitr::include_graphics("./figures/compare_stat.png")
```
...
//table, compare 1feature vs 5feature
